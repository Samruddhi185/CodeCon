...
# some words may be in ormat adipika_sanjay_guhe so separate thm consider ach word count in counter_ingr and then update the value
#in mydict n return 
...
def build_vocab_recipe1m(args):
    ...
    if os.path.exists(ingrs_file) and os.path.exists(instrs_file) and not args.forcegen:
        ...
        counter_toks = pickle.load(open(args.save_path + 'allwords_count.pkl', 'rb'))# it is a list containing cout of unique words in file 
    else:
        ...
        for i, entry in tqdm(enumerate(layer1)):
            ...
            det_ingrs = dets[idx2ind[entry['id']]]['ingredients'] #storing ingredients as 0 th index 1th index

            valid = dets[idx2ind[entry['id']]]['valid'] #storing vali (true,false) in 0th index...
            det_ingrs_filtered = []

            for j, det_ingr in enumerate(det_ingrs):
                if len(det_ingr) > 0 and valid[j]:
                    det_ingr_undrs = get_ingredient(det_ingr, replace_dict_ingrs)#in ingredients replacing & n with and , punctuations with ''
                    det_ingrs_filtered.append(det_ingr_undrs) #filtered data
                    ingrs_list.append(det_ingr_undrs) #final processed list of ingredients 

            # get raw text for instructions of this entry
            acc_len = 0 #total number of words in processed instructions
            for instr in instrs:
                instr = instr['text']
                instr = get_instruction(instr, replace_dict_instrs) #filtering instructions just as ingredients
                if len(instr) > 0:
                    instrs_list.append(instr) #final processed instructions
                    acc_len += len(instr)
            ...
            update_counter(instrs_list, counter_toks, istrain=entry['partition'] == 'train') # counter_toks get pdated by processed inistructions
            title = nltk.tokenize.word_tokenize(entry['title'].lower())
            if entry['partition'] == 'train':
                counter_toks.update(title) # counter_toks get updated by words in title 
            if entry['partition'] == 'train':
                counter_ingrs.update(ingrs_list) # same As counter_toks (instructions) but for indredients
    ...
    counter_ingrs, cluster_ingrs = cluster_ingredients(counter_ingrs) # removing dipika_sanjay_gue kinds of words
    counter_ingrs, cluster_ingrs = remove_plurals(counter_ingrs, cluster_ingrs) # removing nuts and nut count 
    ...
    for i, entry in tqdm(enumerate(layer1)):
        ...
        for instr in instrs_list:
            tokens = nltk.tokenize.word_tokenize(instr) #words are extraced from instrctions
            toks.append(tokens)
